...
import runnerAndSteps.BROWSERS;
import runnerAndSteps.StepImplementations;

public class DBUtilities extends XPathGenerator implements Extras {
	// Constructor.
	...
	// Will take 2 args from cucumber and one from page and then compare both messages.
	public static void checkElement(String arg1, String arg2) {
		Assert.assertEquals(arg1, arg2);		
		System.out.println("Successfully displayed element" + arg2);
	}

	// Will take 2 args both strings and then compare them, basically used for text on UI.
	...
	// Will check if the text element exist on the screen.
	...
	// Will check if the text element does NOT exist on the screen.
	...
	// Different from above this checks for web element.
	...
	// Enter input field the abopve will read the cucumber table and input values on screen as per provided in table.
	...
	public void enterCucumbertableValuesInUI(DataTable table) throws InterruptedException {
		...
		for (int i = 1; i < data.size(); i++) {
			...
			if (name.equals("")) {
				...
				// driver.findElement(By.xpath(myXpath)).click();
				// Disabled above as we have already clicked it before following is to make the input parameters unique to avoid duplocation error on UI.
	...
	// Popup handlers.
	public void checkPopUpMessage(String arg1, BROWSERS browser_type) {
		...
		if (browser_type == BROWSERS.CHROME) {
			...
			// Firefox has a habit of changing the popup messages automatically the message below is the default Firefox popup message.
	...
	public void checkUIElementTableIsDisplayed(String arg1) throws InterruptedException {
		...
		Assert.assertTrue("Verification failed as table NOT FOUND.", myTable.isDisplayed());
	}
